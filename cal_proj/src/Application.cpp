...
long int Application::getFirstBakerStart() const{
...
void Application::preProcessGraphFloyd() {
    ...
    removeNotReachedFromBakeryFloyd(bestPaths, distances, vec);

    removeNotReachedToBakeryFloyd(bestPaths, distances, vec);
...
void Application::dijkstraBacktracking(vector<Client * > sortedClients ,vector<Client * > currClients,
                                       long int currPathTime, long int currTime ,vector<Client * > &bestClients,
                                       long int &bestPathTime){
...
std::string Application::printPath() {
    ...
    for(Baker* baker: bakerSet){
        ...
        path << endl << endl;
        path << "Baker " << baker->getId() << " delivers to " << baker->getClientsAttended().size()
            << " client(s) with a path which has a duration of " << secondsToTime(baker->getPathDuration()) << endl;
        ...
        for(Edge<int>* edge: baker->getPath()){
            ...
            if(c != nullptr && c->getId() == (*it)->getId()){
                int delay = c->getActualDeliveryTime()-c->getScheduledTime()<0 ? 0 : c->getActualDeliveryTime()-c->getScheduledTime();
                path << " has Client " << c->getId() << " and arrived at " << secondsToTime(c->getActualDeliveryTime())
                    << " with a delay of " << secondsToTime(delay) <<"." << endl;
