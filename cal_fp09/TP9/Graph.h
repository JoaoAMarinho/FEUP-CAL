...
double Graph<T>::minCostFlow(T source, T sink, double flow) {
    ...
    Vertex<T>* begin = findVertex(source);
    Vertex<T>* end = findVertex(sink);
    ...
    while (flowT < flow){
        if(firstIter){
            bellmanFordShortestPath(begin);
            firstIter = false;
        }
        else dijkstraShortestPath(begin);

        if(end->dist == INF) break;
        unit_cost += end->dist;

        for(Vertex<T>* vertex : vertexSet){
            for (Edge<T>* edge: vertex->getOutgoing()) {
                edge->cost = edge->cost + edge->orig->dist - edge->dest->dist;
            }
        }


        double f = findMinResidualAlongPath(begin, end);
        f = min(f, flow-flowT);
        augmentFlowAlongPath(begin, end, f);
        flowT += f;
        costT += unit_cost*f;
    }
