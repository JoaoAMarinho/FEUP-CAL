...
Result nearestPoints_BF(std::vector<Point> &vp) {
    ...
    Point point1 = vp[0];
    Point point2 = vp[1];
    double dmin=point1.distance(point2);
    for (int i = 0; i < vp.size(); ++i) {
        for (int j = i+1; j < vp.size(); ++j) {
            if(vp[i].distance(vp[j]) < dmin){
                point1=vp[i];
                point2=vp[j];
                dmin=point1.distance(point2);
            }
        }
    }
    return Result(dmin,point1,point2);
}
...
//Added func
Result nearestPoints_DC_SortedX(std::vector<Point> vp) {
    std::vector<Point> left(vp.begin(),vp.begin()+(vp.size()/2));
    std::vector<Point> right(vp.begin()+(vp.size()/2),vp.end());
    double strip = left[left.size()-1].x;
    if(left.size()==1 && right.size()==1){
        return Result(left[0].distance(right[0]),left[0],right[0]);

    }else{
        Result testResLeft=nearestPoints_DC_SortedX(left);
        Result testResRight=nearestPoints_DC_SortedX(right);
        Result testRes;
        if(testResLeft.dmin<testResRight.dmin) testRes = Result(testResLeft.dmin,testResLeft.p1,testResLeft.p2);
        else testRes = Result(testResRight.dmin,testResRight.p1,testResRight.p2);
        std::vector<Point> inStrip;
        for (int i = 0; i < left.size(); ++i) {
            if(left[i].x>(strip-testRes.dmin))
                inStrip.push_back(left[i]);
        }
        for (int i = 0; i < right.size(); ++i) {
            if(right[i].x<(strip+testRes.dmin))
                inStrip.push_back(right[i]);
        }
        sortByY(inStrip,0,inStrip.size()-1);
        for (int i = 0; i < inStrip.size(); ++i) {
            for (int j = i+1; j < inStrip.size(); ++j) {
                if(abs(inStrip[i].y-inStrip[j].y)>testRes.dmin)
                    break;
                else{
                    if(inStrip[i].distance(inStrip[j])<testRes.dmin){
                        testRes.dmin=inStrip[i].distance(inStrip[j]);
                        testRes.p1=inStrip[i];
                        testRes.p2=inStrip[j];
                    }
                }
            }
        }
...
Result nearestPoints_DC(std::vector<Point> &vp) {
    sortByX(vp, 0, vp.size()-1);
    //TODO
    return nearestPoints_DC_SortedX(vp);
}
Result nearestPoints_DC_MultiThread(std::vector<Point> vp,int nThreads) {
    std::vector<Point> left(vp.begin(),vp.begin()+(vp.size()/2));
    std::vector<Point> right(vp.begin()+(vp.size()/2),vp.end());
    double strip = left[left.size()-1].x;
    if(left.size()==1 && right.size()==1){
        ...
        Result testResRight;
        if(nThreads==1){
            testResLeft=nearestPoints_DC_SortedX(left);
            testResRight=nearestPoints_DC_SortedX(right);
        }else{
            std::thread t1([&](){testResLeft = nearestPoints_DC_MultiThread(left, nThreads / 2);});
            std::thread t2([&](){testResRight = nearestPoints_DC_MultiThread(right, nThreads / 2);});
            t1.join();
            t2.join();
        }
        Result testRes;
...
Result nearestPoints_DC_MT(std::vector<Point> &vp) {
    ...
    sortByX(vp, 0, vp.size()-1);
    //TODO
    return nearestPoints_DC_MultiThread(vp,numThreads);
}
