...
bool Graph<T>::addEdge(const T &sourc, const T &dest, double w) {
    ...
    Vertex<T> * v;
    Vertex<T> * d;
    if ((v = findVertex(sourc))!=NULL && (d=findVertex(dest))!=NULL){
        v->addEdge(d,w);
        return true;
    }
    return false;
...
bool Vertex<T>::removeEdgeTo(Vertex<T> *d) {
    ...
    for (auto e=adj.begin(); e < adj.end(); e++) {
        if((*e).dest->info==d->info){
            adj.erase(e);
            return true;
        }
    }
...
std::vector<T> Graph<T>::topsort() const {
    ...
    std::queue<Vertex<T>*> vertexQueue;
    for ( Vertex<T>* v:vertexSet )
        if (v->indegree==0) {vertexQueue.push(v);}

    std::vector<T> tempVect;
    while (!vertexQueue.empty()){
        Vertex<T> * v = vertexQueue.front();
        vertexQueue.pop();
        tempVect.push_back(v->info);
        for (Edge<T> e : v->adj) {
            e.dest->indegree--;
            if(e.dest->indegree==0) vertexQueue.push(e.dest);
        }
    }
    if(tempVect.size()!=vertexSet.size()) return std::vector<T>();
    return tempVect;
}
...
int Graph<T>::maxNewChildren(const T & source, T &inf) const {
    ...
    int currentMax=0, testMax=0;
    for (Vertex<T>* v:vertexSet)
        v->visited=false;

    Vertex<T>* v = findVertex(source);
    v->visited=true;
    currentMax=v->adj.size();
    inf=v->info;
